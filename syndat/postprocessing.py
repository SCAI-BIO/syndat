...
def normalize_scale(real_df: pd.DataFrame, synthetic_df: pd.DataFrame) -> pd.DataFrame:
    ...
    :param real_df: The real DataFrame.
    :param synthetic_df: The synthetic DataFrame.

    :return: The scaled synthetic DataFrame.
    """
...
def assert_minmax(real: pd.DataFrame, synthetic: pd.DataFrame, method: str = 'clip') -> pd.DataFrame:
    ...
    :param method: Either 'delete' or 'clip'. 'Delete' will remove records that
    fall outside the min-max range, while 'clip' will adjust the values to fit within the range.

    :return: The post-processed synthetic dataset.
    """
