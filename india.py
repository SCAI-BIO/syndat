...
sdt_Enc0 = pd.read_csv(os.path.join(fold_path,'Sims_Stat_S_Enc0_EP64.csv'), na_values='.')
ldt_Enc0 = pd.read_csv(os.path.join(fold_path,'Sims_Long_Enc0_EP64.csv'), na_values='.')
...
lt.loc[lt["Variable"] == "death", "Type"] = "cat"
lt.loc[lt["Variable"] == "death", "Cats"] = 2
...
long_cat_metrics_micro = compute_long_categorical_error_metrics(
    ...
    per_time_mean=True,
    per_variable_mean=True)
gof_continuous_list(rp_Enc0, ldt, strat_vars=["DRUG"], log_trans=True, save_path=fold_path) 
...
raincloud_continuous_list(rp_Enc0, ldt,type='longitudinal',strat_vars=["DRUG"], save_path=fold_path) 
